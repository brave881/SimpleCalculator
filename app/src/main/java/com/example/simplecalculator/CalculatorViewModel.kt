...
import com.example.simplecalculator.domain.ExpressionEvaluator
import com.example.simplecalculator.domain.ExpressionParser
import com.example.simplecalculator.domain.MathsSymbols
...
class CalculatorViewModel(
    ...
    fun onCalculatorEvent(events: CalculatorEvents) {
        ...
        when (events) {
            CalculatorEvents.Evaluate -> {
                viewModelScope.launch(Dispatchers.Default) {
                    try {
                        ...
                        _result.update {
                            if (floor(evaluated) == evaluated) evaluated.toInt().toString()
            ...
            CalculatorEvents.Clear -> if (_expression.value.isNotEmpty()) _expression.update {
                it.dropLast(
                    1
                )
            }
            ...
            is CalculatorEvents.Operation ->
                if (canEnterOperation()) _expression.update { "$it${events.operators.symbol}" }

            is CalculatorEvents.Parenthesis -> processParentheses()
            is CalculatorEvents.Factorial -> if (canEnterFactorial()) _expression.update { "$it!" }

            CalculatorEvents.PI -> processPi()
            is CalculatorEvents.Sqrt -> if (canEnterSqrt()) _expression.update { "$it${MathsSymbols.SQRT_SYMBOL}" }
        }

    }

    private fun canEnterSqrt(): Boolean =
        _expression.value.isEmpty() || _expression.value.isNotEmpty() && _expression.value.last() !in ")!"


    private fun canEnterFactorial(): Boolean =
        _expression.value.isNotEmpty() && _expression.value.last() in "0123456789"
    ...
    private fun processParentheses() {
        ...
        _expression.value += when {
            _expression.value.isEmpty() || expression.value.last() in "${Operators.allOperatorSymbols}(" -> "("

            _expression.value.last() in "0123456789)" && openingCount == closingCount -> return
    ...
    private fun canEnterDecimal(): Boolean =
        _expression.value.isNotEmpty() && expression.value.last() !in "${Operators.allOperatorSymbols}.()" && !_expression.value.takeLastWhile { it in "0123456789." }
            .contains(".")


    private fun canEnterOperation(): Boolean =
        _expression.value.isNotEmpty() && _expression.value.last() in "0123456789)${MathsSymbols.PI_SYMBOL}${MathsSymbols.FACTORIAL_SYMBOL}"
